...
- 복원과 영구 삭제가 되는 휴지통. 30일이 지난 항목은 알아서 사라진다
...
화면 공유와 스크린샷에서 메모를 감출지, Dock 아이콘을 남길지. Dock 아이콘은 기본으로 켜져
있어서 실행 중인 Posteight 를 Dock 과 Command-Tab 으로 부를 수 있고, 누르면 메모 창이 다시
나온다. 끄면 메뉴 막대에만 사는 앱이 된다.
...
Mac 의 다른 무엇도 그 안에 손을 뻗지 못하고, 파일은 본인만 읽을 수 있게 만들어진다.
설정 → 앱 → 폴더 열기로 갈 수 있다. 샌드박스 이전 버전에서 쓰던 메모는 첫 실행에 한 번
복사되고, 예전 폴더는 그대로 남는다.

계정도, 동기화도, 사용 정보 수집도 없다. 내보내기는 명시적으로만 일어난다 — 오늘 기록을
Markdown 으로 클립보드에 복사하는 것이 전부이고, 클립보드 매니저가 히스토리에 남기지
않도록 표식을 붙인다. Notion 동기화는 아직 구현되어 있지 않다.
