...
- Command-Z로 편집을 취소하고 Shift-Command-Z로 다시 실행한다. 메모 창에서 Command-A, Command-C로 현재 탭 전체를 복사할 수 있다. 텍스트 편집 중에는 편집 중인 필드가 우선한다.
- 할 일별로 알림 시간을 예약할 수 있다. 설치된 앱에서 알림 권한을 허용해야 하며, 알림 본문은 기본으로 감춘다.
- 설정에서 글자 크기와 폰트를 고르고, 개인 폰트 파일을 추가할 수 있다.
