        ...
        @Test func searchScrollsToRowAndFocusesAgainThenOpensDetail() async throws {
            ...
            for _ in 0..<2 {
                store.searchFocusRequest = SearchFocusRequest(noteID: noteID, tabID: tab.id,
                                                              target: .itemTitle(item.id), query: "회의")
                let ready = try await waitForEditorState {
                    guard let field = fields(in: host).first(where: { $0.stringValue == "🙂 회의 준비" }),
                          let editor = field.currentEditor() as? NSTextView else { return false }
                    return store.searchFocusRequest == nil && editor.selectedRange() == NSRange(location: 5, length: 0)
                }
                try #require(ready, "검색한 행의 편집기와 커서가 준비되지 않음")
            ...
            let detailReady = try await waitForEditorState {
                store.searchFocusRequest == nil && NSApp.windows.contains {
                    guard let editor = $0.firstResponder as? NSTextView else { return false }
                    return editor.string == "지난 회의 기록" && editor.selectedRange() == NSRange(location: 5, length: 0)
                }
            }
            try #require(detailReady, "세부사항 편집기와 커서가 준비되지 않음")
